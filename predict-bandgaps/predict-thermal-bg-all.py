...
XDATCAR_path = 'MD-files/'                                              # Path to the XDATCAR files
...
list_materials = ['Ag3SBr', 'Ag3SBr0_125I0_875', 'Ag3SBr0_25I0_75', 
                   'Ag3SBr0_375I0_625', 'Ag3SBr0_5I0_5', 
                   'Ag3SBr0_625I0_375', 'Ag3SBr0_75I0_25', 
                   'Ag3SBr0_875I0_125', 'Ag3SI']                        # List of considered materials
...
for material in list_materials:
    ...
    get_initial_POSCAR(XDATCAR_path + material + '/' + '100/XDATCAR', outputs_dir + material + '/')
    ...
    for temperature in list_temperatures:
        if os.path.exists(outputs_dir + material + '/' + temperature):
            shutil.rmtree(outputs_dir + material + '/' + temperature)
        os.mkdir(outputs_dir + material + '/' + temperature)

        # Generate the POSCAR files from the MD results
        generate_POSCAR_from_snapshots(XDATCAR_path + material + '/' + temperature +'/XDATCAR', 
                                       starting_point, number_of_snapshots, total_configurations, 
                                       outputs_dir + material + '/' + temperature + '/')
        ...
        for struc in range(number_of_snapshots):
            # Generate the graph
            try:
                poscar = Poscar.from_file(outputs_dir + material + '/' + temperature + '/POSCAR-' + str(struc + 1).zfill(3))
                structure_object = poscar.structure 
        ...
        # Save the values
        with open(outputs_dir + material + '/' + temperature + '/prediction.txt', 'w') as file:
