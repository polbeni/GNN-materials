...
class GCNN(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, batch):
        ...
        # Node embedding
        x = self.conv1(x, edge_index, edge_attr[:, 0].unsqueeze(1))
        x = self.bn1(x)
        x = x.relu()
        x = self.conv2(x, edge_index, edge_attr[:, 1].unsqueeze(1))
        x = self.bn2(x)
        x = x.relu()
        x = self.conv3(x, edge_index, edge_attr[:, 2].unsqueeze(1))
        x = self.bn3(x)
        x = x.relu()
        x = self.conv4(x, edge_index, edge_attr[:, 3].unsqueeze(1))
        x = self.bn4(x)
        x = x.relu()
...
model = GCNN(features_channels=dataset_graphs[0].num_node_features, hidden_channels=256)
print(model)
